    ...
    <section className="py-20 capitalize bg-[#050505] overflow-hidden">
      <div className="container px-6 mx-auto">
        ...
        <div className="flex flex-col items-end justify-between gap-8 mb-12 md:flex-row">
          <div className="max-w-2xl">
            ...
            <p className="font-medium leading-relaxed tracking-wide text-gray-100 max-w-lg">
              Hand-picked luxury itineraries designed for those who seek the extraordinary in every journey.
            </p>
          </div>
          
          <Link href="/packages" className="group hidden lg:flex items-center gap-3 text-[11px] font-black tracking-[0.3em] uppercase text-white hover:text-yellow-500 transition-all">
        ...
        <div className="mt-4 text-center md:hidden">
            <Link href="/packages" className="inline-flex items-center gap-3 px-10 py-5 text-[10px] font-black tracking-[0.3em] uppercase bg-white/5 border border-white/10 rounded-full text-white active:bg-yellow-500 active:text-black transition-all">
               Explore Collections
            </Link>
        </div>
      </div>
          ...
          <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
            ...
            >
              <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={() => setIsModalOpen(false)} className="text-gray-400 hover:text-yellow-500 transition-colors">
                  <X size={24} />
                </button>
              </div>
              ...
              <form onSubmit={handleWhatsApp} className="space-y-4">
                <div className="p-3 bg-yellow-500/10 border border-yellow-500/20 rounded-xl">
                  <p className="text-[10px] uppercase font-bold text-yellow-500 tracking-widest mb-1">Selected Journey</p>
                  <p className="text-sm font-black text-white uppercase">{selectedPackage}</p>
                </div>

                <div className="grid grid-cols-2 gap-4">
                  <input type="text" placeholder="Your Name" required className="booking-input" onChange={(e) => setFormData({...formData, name: e.target.value})} />
                  <input type="tel" placeholder="Mobile Number" required className="booking-input" onChange={(e) => setFormData({...formData, mobile: e.target.value})} />
                </div>

                <div className="grid grid-cols-3 gap-4">
                  <div className="col-span-1">
                    <label className="text-[9px] uppercase font-bold text-gray-500 mb-1 block">Date</label>
                    <input type="date" required className="booking-input py-[9px]" style={{colorScheme: 'dark'}} onChange={(e) => setFormData({...formData, date: e.target.value})} />
                  </div>
                  <div>
                    <label className="text-[9px] uppercase font-bold text-gray-500 mb-1 block">Adults</label>
                    <input type="number" min="1" placeholder="0" className="booking-input" onChange={(e) => setFormData({...formData, adults: e.target.value})} />
                  </div>
                  <div>
                    <label className="text-[9px] uppercase font-bold text-gray-500 mb-1 block">Kids</label>
                    <input type="number" min="0" placeholder="0" className="booking-input" onChange={(e) => setFormData({...formData, kids: e.target.value})} />
                  </div>
                </div>

                <textarea placeholder="Special Requirements / Message" rows="3" className="booking-input resize-none w-full" onChange={(e) => setFormData({...formData, message: e.target.value})}></textarea>
